...
- 原始音频和本地转录记录保留在这台 Mac；音频不会发送给 DeepSeek。
- 只有主动生成总结时，转录、会议元数据和书签上下文才会发送给 DeepSeek。
- DeepSeek 只返回总结；Notion 自动归档开启时，App 再将这份总结连同本地书签上下文和完整转录写入用户指定的 Notion 页面。
- Notion 自动归档默认开启。关闭后，生成总结流程只保存本地总结，不会自动联系 Notion；设置中的“测试连接”仍会主动连接 Notion。
- DeepSeek API Key 与 Notion Token 存在 Keychain；普通设置保存模型名、Notion 父页面链接和自动归档开关。
...
5. Notion 自动归档默认开启。选择是否自动归档后点击“保存设置”；开启时，生成总结后会联系 Notion，并在该父页面下创建独立会议子页面。
6. 关闭自动归档后，生成总结流程只保存本地总结，不会自动联系 Notion；手动点击“测试连接”仍会连接 Notion。
7. 若已有本地总结，重新开启自动归档并保存设置后，返回会议详情点击“归档到 Notion”。这不会重新生成总结，也不会再次调用 DeepSeek。
...
- `/tmp/MeetingNotes-default-store-before-pin-20260717-115107.store` 只是旧 `default.store` 主文件的保护性副本；由于 SQLite 使用 WAL 模式，单独复制主文件不能作为完整逻辑备份。一致快照以 SQLite `.backup` 生成的 `/tmp/MeetingNotes-consistent-before-manual-fixes-20260717-124928.store` 为准：`PRAGMA quick_check=ok`，包含 1 条会议、0 条转录，SHA-256 为 `17f2562833b2d160e2f52adf83dac3a8cca4f57a419fe5f397eac86685bd7c8f`。
